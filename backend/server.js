...
app.post('/api/group', async (req, res) => {
  ...
  data[code] = {
    ...
    members: [],
    expenses: [],
    categories: ['Food', 'Travel', 'Accommodation', 'Entertainment', 'Other']
  };
...
app.post('/api/group/:code/member', async (req, res) => {
  ...
  const memberId = 'm_' + Math.random().toString(36).substring(2, 11);
  group.members.push({
    id: memberId,
    name,
...
app.post('/api/group/:code/expense', async (req, res) => {
  const { title, amount, category, payerId, splitType, splits, date } = req.body;
  const { code } = req.params;
...
app.put('/api/group/:code/expense/:expenseId', async (req, res) => {
  ...
  group.expenses[expenseIndex] = {
    ...
    splitType,
    splits,
    date: date || group.expenses[expenseIndex].date
  };
...
app.post('/api/group/:code/category', async (req, res) => {
  ...
  if (!group.categories) {
    group.categories = ['Food', 'Travel', 'Accommodation', 'Entertainment', 'Other'];
  }
...
app.delete('/api/group/:code/category/:categoryName', async (req, res) => {
  ...
  group.categories = group.categories.filter(c => c !== categoryName);
